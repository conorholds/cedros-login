...
import { u as R } from "./useSystemSettings-rgskaDqP.js";
function E(e) {
  const s = Math.floor(e / 86400), a = Math.floor(e % 86400 / 3600), n = Math.floor(e % 3600 / 60), i = e % 60;
  return { days: s, hours: a, minutes: n, seconds: i };
}
function I(e) {
  const { days: s, hours: a, minutes: n } = E(e), i = [];
  return s > 0 && i.push(`${s}d`), a > 0 && i.push(`${a}h`), n > 0 && i.push(`${n}m`), i.length === 0 && i.push(`${e}s`), i.join(" ");
}
const U = {
...
function N(e) {
  ...
  return s.length === 1 ? e : s.map((a, n) => {
    const i = a.match(/^<a\s+href="([^"]+)"[^>]*>([^<]+)<\/a>$/);
    return i ? /* @__PURE__ */ t("a", { href: i[1], target: "_blank", rel: "noopener noreferrer", children: i[2] }, n) : a;
  });
}
function Q({
  settings: e,
  ...
  return /* @__PURE__ */ t("div", { className: "cedros-settings-grid", children: e.map((i) => /* @__PURE__ */ t(
    M,
    {
      ...
      editValue: s[i.key],
      onChange: a,
      externalWarning: n?.[i.key]
    },
    i.key
  )) });
}
function M({ setting: e, editValue: s, onChange: a, externalWarning: n }) {
  const i = U[e.key], l = s ?? e.value, p = s !== void 0 && s !== e.value, r = i?.inputType === "boolean", o = S(() => {
    if (n) return n;
...
function F({ value: e, onChange: s, presets: a, min: n = 0 }) {
...
function W({ value: e, onChange: s, presets: a, unit: n }) {
  const i = !a.some((r) => r.value === e), l = d(
    (r) => {
      r.target.value !== "__custom__" && s(r.target.value);
    },
    [s]
  ), p = d(
    (r) => {
...
function $({ value: e, onChange: s, min: a, max: n, unit: i }) {
  const l = d(
    (p) => {
      s(p.target.value);
    },
...
function J({ value: e, onChange: s, multiline: a }) {
  const [n, i] = w(!1), [l, p] = w(!1), r = e && e.length > 0, o = d(() => {
    i(!0), p(!0);
  }, []), c = d(() => {
    i(!1), p(!1);
  }, []), m = d(
...
function G({ value: e, onChange: s }) {
  ...
  return /* @__PURE__ */ u("div", { className: "cedros-token-list-input", children: [
    /* @__PURE__ */ u("div", { className: "cedros-token-presets", children: [
      ...
      /* @__PURE__ */ t("div", { className: "cedros-token-presets-list", children: A.map((r) => /* @__PURE__ */ t("span", { className: "cedros-token-preset-chip", children: r }, r)) })
    ] }),
...
function B({ value: e, onChange: s }) {
  const a = S(() => e.split(",").map((l) => l.trim()).filter(Boolean), [e]), n = d(
    (l) => {
...
const H = 800, V = 2e3;
function Z() {
  const { settings: e, isLoading: s, error: a, fetchSettings: n, updateSettings: i } = R(), [l, p] = w({}), [r, o] = w("idle"), [c, m] = w(null), y = T(null), _ = T(null), k = T({});
  O(() => () => {
