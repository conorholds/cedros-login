import { jsx as ee, jsxs as Ne } from "react/jsx-runtime";
import { useMemo as v, useEffect as P, useState as D, useRef as O, useCallback as d } from "react";
import { A as ke, g as Ie, a as De, h as V, u as Ue, b as Pe, c as Ye, C as xe } from "./useCedrosLogin-BDbp-ld1.js";
...
function Ge(e = {}) {
  const t = me(), A = Xe(e.env), i = { ...t };
  for (const r of Object.keys(_))
    i[r] = e.config?.[r] ?? A[r] ?? e.base?.[r] ?? t[r];
  return i;
}
function We(e, t = {}) {
  return t.flags ? t.flags[e] ?? _[e].defaultEnabled : Ge(t)[e];
}
function je() {
  return be().filter((e) => e.autoDiscoverable).map((e) => e.name);
}
function Ze() {
  const e = me(), t = {};
  for (const A of je())
...
function ve(e, t) {
  ...
  for (let i = 0; i < A; i++) {
    const r = i << 1;
    e[i] = Je(t.charCodeAt(r), t.charCodeAt(r + 1));
  }
}
function kt(e, t) {
  ...
  for (let A = 0; A < t.length; A++) {
    const i = A << 1;
    if (t[A] !== Je(e.charCodeAt(i), e.charCodeAt(i + 1)))
      return !1;
  }
  return !0;
}
...
function Ve(e, t) {
  return j(this, void 0, void 0, function* () {
    ...
    const n = (a, I = 0) => {
      ...
      return ve(u, e.hash), u.set(w, q), u;
    }, m = (a) => {
...
function Yt(e) {
  return j(this, void 0, void 0, function* () {
    ...
    ve(y, E.calculate(new Uint8Array([]), o));
...
function oA() {
  const { config: e, user: t } = Ue(), { status: A, solanaPubkey: i, hasExternalWallet: r } = rA(), n = e.wallet?.exposeAvailability ?? !1, s = e.wallet?.exposePublicKey ?? !1;
  return P(() => {
    if (!n || !t) {
      he();
      return;
...
function kA({ config: e, children: t }) {
  ...
  return o && U ? null : /* @__PURE__ */ ee(Pe.Provider, { value: x, children: /* @__PURE__ */ ee(Ye.Provider, { value: W, children: /* @__PURE__ */ ee(xe.Provider, { value: Me, children: /* @__PURE__ */ Ne("div", { className: l.className, style: l.style, children: [
    /* @__PURE__ */ ee(oA, {}),
    t
  ] }) }) }) });
}
function DA() {
  const { user: e, authState: t, error: A, logout: i, refreshUser: r, openModal: n, closeModal: s } = Ue();
  return {
    user: e,
    authState: t,
    error: A,
