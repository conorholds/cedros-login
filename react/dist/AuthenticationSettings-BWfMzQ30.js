...
import { u as T, A as j, S as F } from "./AutosaveStatus-Bgts5i6l.js";
...
function U() {
  ...
  const [v, w] = y([]), [f, N] = y(0), [C, p] = y(!1), [k, a] = y(null), n = _(() => {
    const b = g.current();
    return {
...
function H({ className: t }) {
  const {
    ...
    deleteProvider: N,
    toggleProvider: C
  } = U(), { activeOrg: p } = B(), [k, a] = y("list"), [n, o] = y(null), [i, h] = y(null);
  $(() => {
    v(p?.id);
  }, [v, p?.id]);
  const x = () => {
    ...
    h(null);
    try {
      k === "add" ? await w(r) : n && await f(n.id, r), a("list"), o(null);
    } catch (S) {
...
function W({
  ...
  const f = !!t, [N, C] = y(t?.name ?? ""), [p, k] = y(t?.issuerUrl ?? ""), [a, n] = y(t?.clientId ?? ""), [o, i] = y(""), [h, x] = y(t?.emailDomain ?? ""), [E, b] = y(t?.allowRegistration ?? !0), [c, d] = y(t?.enabled ?? !0), u = _(
    (r) => {
...
function ee({ settings: t, handleChange: m }) {
  const [l, g] = y(() => Y(t)), v = M(null), [w, f] = y(null), N = _(
    (n) => {
...
const I = [
  ...
  { id: "buttonorder", label: "Button Order", categories: [], isCustom: !0 },
  {
    id: "postlogin",
    label: "Post-Login",
    categories: ["postlogin", "postlogin.welcome", "postlogin.username", "postlogin.complete"],
    keys: ["postlogin_redirect_url", "postlogin_welcome_enabled", "postlogin_welcome_route", "postlogin_username_enabled", "postlogin_complete_enabled"]
  }
];
